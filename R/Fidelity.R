...
#' taxon_index = Intermediate result (`comm_name` `iv.max` `p.value` `taxon_index` `most_loyal_to`), taxa that are more host specific will have a greater taxon_index. The column "most_loyal_to" indicates the group for which that taxon had the highest loyalty;
...
#'
#' First, the permutation test of the ISA is used to assign a specificity index to each taxon. These values are generated by subtracting 1 from the p-values generated by the ISA, with values closer to 1 indicating taxa that are more specific. The analysis is based on the ISA described by Dufrêne & Legendre (1997) and the permutation test is based on that described by McCune & Grace (2002). The taxon_index output provides a list of these taxa level indices.
#'
#' Community level specificity indices are then generated through CWM analysis using taxon level specificity indices. Values closer to one indicate communities that are more host specific. This output can be found in the community_index output.
#'
...
Fidelity <-
  ...
  if(to_remove[1] > 1 & rm.rare.taxa==FALSE) {
  ...
  if (ovp.plot && occurrence_cutoff > 0) {
    p <-
      ...
      labs(x = "Number of sample occurrences", y = "P value", color = "Indicator\nvalue") +
  ...
  out <- list(community_index = output,
              taxon_index = indicator_results,
